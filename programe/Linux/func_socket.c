...
void socket_accept(int fd_sock,struct sockaddr *addr,int *newfd)
{
	int addrlen = sizeof(struct sockaddr);
	*newfd = accept(fd_sock,addr,&addrlen);
	if(-1 == *newfd)
	{
		perror("accept");
		exit(0);
...
void socket_client_udp(int *fd_sock,int port,char *ipchar,struct sockaddr *ip_addr)
{
	*fd_sock = socket(AF_INET,SOCK_DGRAAM,0);
	if(-1 == *fd_sock)
	{
		perror("socket");
		exit(0);
